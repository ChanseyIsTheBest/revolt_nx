...
/* Top-level directories kept OUT of the pack.
 *
 * The pack is read-only and libc_shim consults it before the real filesystem,
 * so anything packed shadows the on-disk copy forever. RVGL writes profiles,
 * replays, lap times and its shader cache into these, and a packed profiles/
 * would make every save appear to succeed and then read back the packed
 * version. They stay on the card. */
static const char *const kNeverPack[] = {
  "profiles", "replays", "times", "cache", "packs", "logs", "screenshots",
};

static int excluded_from_pack(const char *relative) {
  for (size_t i = 0; i < sizeof kNeverPack / sizeof kNeverPack[0]; i++) {
    const size_t n = strlen(kNeverPack[i]);
    if (strncmp(relative, kNeverPack[i], n) == 0 &&
        (relative[n] == '\0' || relative[n] == '/'))
      return 1;
  }
  return 0;
}
...
static int collect_files(const char *root, const char *relative,
  ...
  while (ok && (entry = readdir(dir)) != NULL) {
    ...
    if (excluded_from_pack(child_relative)) continue;
